...
def findBH(snap):
    BHfilter = np.where(snap.stars['iord']==89425759)
...
for halo in uniqueBHhalos:
    ...
    # profile for stars
    pstar = pynbody.analysis.profile.Profile(h[halo].s,min=.01,max=maxr,nbins= nbins,ndim=3,type='log')
    # profile for gas
    pgas = pynbody.analysis.profile.Profile(h[halo].g,min=.01,max=maxr,nbins= nbins,ndim=3,type='log')
    # profile for all matter (star, gas, dark)
    pall = pynbody.analysis.profile.Profile(h[halo],min=.01,max=maxr,nbins= nbins,ndim=3,type='log')

    # set up the plot
    f, axs = plt.subplots(1,2,figsize=(14,6)) # 1,3 makes 1 row, 3 columns
   ...
   # axs[1].set_ylabel(r'$\rho_{gas}$ [M$_{\odot}$ kpc$^{-2}$')
    ...
    axs[1].plot(pall['rbins'],pall['density'],'k')
    axs[1].semilogx()
    axs[1].semilogy()
    axs[1].set_xlabel('R [kpc]')
    axs[1].set_ylabel(r'$\rho_{total}$ [M$_{\odot}$ kpc$^{-2}$')
